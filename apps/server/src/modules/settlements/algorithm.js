...
exports.calculateNaiveTransactions = calculateNaiveTransactions;
exports.minimizeCashFlow = minimizeCashFlow;
exports.generateGroupSummary = generateGroupSummary;
/**
...
function calculateNetBalances(members, expenses, settlements) {
    ...
    for (const expense of expenses) {
        ...
        if (expense.status === 'disputed')
            continue;
        // Credit the payer
        const payerId = expense.paidById || expense.paidBy;
 ...
 */
function calculateNaiveTransactions(expenses, settlements, membersMap, currency) {
    // Map of pairwise debt: `${fromId}->${toId}` => amount
    const debtPairMap = new Map();
    for (const expense of expenses) {
        if (expense.status === 'disputed')
            continue;
        const payerId = expense.paidById || expense.paidBy;
 ...
 */
function minimizeCashFlow(netBalances, currency) {
 ...
 */
function generateGroupSummary(members, expenses, settlements, donations, currency, groupId) {
    const membersMap = new Map(members.map(m => [m.id, m]));
    const balanceMap = calculateNetBalances(members, expenses, settlements);
    const netBalancesList = Array.from(balanceMap.values());
    const balances = netBalancesList.map(b => ({
        userId: b.user.id,
        user: b.user,
